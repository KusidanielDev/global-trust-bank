...
async function main() {
  ...
  // accounts
  const a1 = await prisma.account.create({ data: { userId: user.id, name: 'Primary Checking', type: 'checking', currency: 'USD', accountNumber: Array.from({length:12},()=>Math.floor(Math.random()*10)).join('') } });
  const a2 = await prisma.account.create({ data: { userId: user.id, name: 'Primary Checking', type: 'checking', currency: 'USD', accountNumber: Array.from({length:12},()=>Math.floor(Math.random()*10)).join('') } });
  // transactions
  const now = new Date();
